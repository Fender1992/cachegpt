...
**Latest Update**: PDF Upload Fix with pdfjs-dist

### What's New in v12.15.0:
1. ✅ **PDF Upload Fix** (October 31, 2025)
   - Fixed "DOMMatrix is not defined" error in /api/upload
   - Replaced pdf-parse with pdfjs-dist (official Mozilla PDF.js library)
   - pdfjs-dist v3.11.174 works reliably in Node.js server environments
   - Extracts text from all PDF pages sequentially
   - No test file dependencies or browser API requirements

### What Was Fixed:
- ❌ **pdf-parse incompatibility**: All versions had issues (2.4.5 = DOMMatrix, 1.1.1 = test file dependencies)
- ✅ **Root cause**: pdf-parse uses browser APIs and has broken dependency chains
- ✅ **Solution**: Switched to pdfjs-dist, the official Mozilla library designed for both browser and Node.js
- ✅ **File location**: app/api/upload/route.ts:3, 198-237
...
### Package Changes:
- Removed: `pdf-parse` (all versions problematic)
- Added: `pdfjs-dist@3.11.174` (official Mozilla library)
