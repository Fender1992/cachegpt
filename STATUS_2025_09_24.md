...
**Latest Update**: PDF Upload Fix with pdf2json + Unified Auth
...
2. ✅ **Upload Authentication Fix** (October 31, 2025)
   ...
   - Migrated from cookie-only auth to unified auth resolver
   - Now supports both cookies (web) and Bearer tokens (mobile/CLI)
   - Matches authentication pattern used in /api/conversations endpoints
...
- ❌ **Authentication incompatibility**: Cookie-only auth doesn't work for mobile/CLI clients
- ✅ **Root cause PDF**: Libraries with native dependencies (canvas, node-gyp) can't build in serverless environments
- ✅ **Root cause Auth**: Upload endpoint wasn't using unified auth resolver like other endpoints
- ✅ **Solution PDF**: Switched to pdf2json, pure JavaScript PDF parser with no native dependencies
- ✅ **Solution Auth**: Implemented unified auth resolver (supports cookies, Bearer tokens, API keys)
- ✅ **File location**: app/api/upload/route.ts:1-52, 198-236
...
// ✅ Works with cookies (web), Bearer tokens (mobile/CLI), and API keys
```
