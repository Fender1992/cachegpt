...
**Latest Update**: PDF Upload DOMMatrix Error Fix

### What's New in v12.15.0:
1. ✅ **PDF Upload Fix** (October 31, 2025)
   - Fixed "DOMMatrix is not defined" error in /api/upload
   - Downgraded pdf-parse from 2.4.5 to 1.1.1 (stable Node.js compatible version)
   - Updated import from `{ PDFParse }` to default import `pdf`
   - Updated API usage from `new PDFParse()` to direct `pdf()` function call
   - Added @types/pdf-parse for TypeScript support

### What Was Fixed:
- ❌ **pdf-parse 2.4.5 incompatibility**: Version 2.4.5 uses browser APIs (DOMMatrix) not available in Node.js
- ✅ **Root cause**: Canvas/DOMMatrix dependencies in newer pdf-parse versions
- ✅ **Solution**: Version 1.1.1 is pure Node.js compatible without browser dependencies
- ✅ **File location**: app/api/upload/route.ts:3, 198-219

### Technical Details:
**Before** (pdf-parse 2.4.5):
```typescript
...
### Package Changes:
- `pdf-parse`: 2.4.5 → 1.1.1
- Added: `@types/pdf-parse@1.1.4` (dev dependency)
