...
## 🔍 VERSION 12.3.3 - October 24, 2025
**Latest Update**: Added Debug Logging for Cookie Authentication Diagnosis

### What's New in v12.3.3:
1. 🔍 **Enhanced Debug Logging**: Added detailed logging to diagnose cookie auth issues
   - Client-side logging in `loadConversations()` shows session state and API responses
   - Server-side logging in `validateCookieSession()` shows cookie presence and validation
   - Helps identify if cookies are being sent/received correctly
2. ⚠️ **Issue Identified**: API returns `requiresAuth: true` despite user being authenticated
   - Frontend successfully gets session from Supabase
   - API call includes `credentials: 'include'`
   - Server-side cookie validation may be failing
3. 🔧 **Troubleshooting**: Debug logs will reveal exact failure point in production
