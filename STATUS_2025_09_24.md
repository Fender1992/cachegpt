...
**Latest Release**: Anthropic SDK Integration for /v1/messages Endpoint

### What's New in v12.1.0:
1. ✅ **Anthropic SDK Dependency**: Added `@anthropic-ai/sdk` package (v0.67.0)
2. ✅ **Build Fixed**: Resolved missing dependency error for `/v1/messages` endpoint
3. ✅ **SDK Compatibility**: Enables full Anthropic SDK integration support

### Technical Details:

**Package Addition**:
- Added `@anthropic-ai/sdk` to support the existing `/v1/messages` API endpoint
- Enables SDK-compatible responses for Claude API integration
- Allows third-party tools to use CacheGPT as an Anthropic API proxy

**Build Status**:
- ✅ Build successful with all 76 static pages generated
- ✅ `/v1/messages` endpoint functional and included in build
- ⚠️ Minor warnings about Supabase Edge Runtime (expected, non-critical)

**Commit**: e0e42c3
**Files Changed**: package.json, package-lock.json, yarn.lock
