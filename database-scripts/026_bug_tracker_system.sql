...
INSERT INTO bugs (title, description, status, priority, category, user_email, steps_to_reproduce, expected_behavior, actual_behavior) VALUES
('Mobile URL bar covers input', 'On mobile devices, the browser URL bar covers the chat input field making it impossible to type', 'resolved', 'high', 'mobile', 'test@example.com', '1. Open chat on mobile\n2. Try to type message', 'Input field should be visible and accessible', 'Input field is covered by URL bar'),
('Chat UI hard to read', 'The dark theme makes it difficult to read messages', 'resolved', 'medium', 'ui', 'test@example.com', '1. Open chat interface\n2. Try to read messages', 'Text should be clearly readable', 'Low contrast makes text hard to read')
