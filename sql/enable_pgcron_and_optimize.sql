...
-- Enable pg_cron extension (you may need to do this in Supabase Dashboard)
...
-- =====================================================
-- STEP 2: FIX COMPRESSION (NO EXTERNAL FUNCTIONS NEEDED)
-- =====================================================

-- Drop any broken functions from previous attempts
...
DROP FUNCTION IF EXISTS compress_text_gzip(TEXT) CASCADE;
...
-- =====================================================
-- STEP 3: PARTITION MAINTENANCE (WORKS WITHOUT CRON)
-- =====================================================
...
-- =====================================================
-- STEP 6: FIX SIMILARITY FUNCTIONS
-- =====================================================
...
-- =====================================================
-- STEP 8: SCHEDULE JOBS (ONLY IF PG_CRON EXISTS)
-- =====================================================
...
BEGIN
  IF EXISTS (
    ...
    BEGIN
      ...
      PERFORM cron.unschedule('compress-responses');
    EXCEPTION
  ...
  ELSE
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  pg_cron not enabled - Jobs not scheduled';
    RAISE NOTICE 'You will need to run these functions manually:';
    RAISE NOTICE '  - SELECT create_monthly_partitions(); -- Monthly';
    RAISE NOTICE '  - SELECT * FROM auto_cleanup_cache(); -- Weekly';
    RAISE NOTICE '  - SELECT archive_valuable_old_data(); -- Monthly';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '✅ Partitions created/verified';
  ...
  -- Archive old data
  RAISE NOTICE '  - Archived: % rows', archive_valuable_old_data();
...
UNION ALL
SELECT
  'Cache Size: ' || pg_size_pretty(pg_total_relation_size('cached_responses'))
UNION ALL
SELECT
  'Hit Rate: ' || ROUND(
    COUNT(CASE WHEN access_count > 1 THEN 1 END)::NUMERIC /
...
UNION ALL
SELECT
  'TOAST Compression Active: Yes (automatic for responses > 2KB)'
UNION ALL
SELECT
  CASE
    WHEN EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron')
    THEN 'Scheduled Jobs: ACTIVE ✅'
    ELSE 'Scheduled Jobs: MANUAL MODE ⚠️ (enable pg_cron for automation)'
  END;