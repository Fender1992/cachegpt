 ...
 * Yahoo uses Basic Auth (base64 of client_id:client_secret) for token exchange.
 */
...
/**
 * Get a valid Yahoo access token, refreshing if expired.
...
export async function getValidYahooToken(
  ...
  if (!refreshToken) {
    console.error('[Yahoo Token] No refresh token available for integration', integrationId);
    return accessToken; // Return expired token as last resort (might still work)
  }

  try {
    console.log('[Yahoo Token] Refreshing expired token for integration', integrationId);
    ...
    console.log('[Yahoo Token] Successfully refreshed token, expires at', newExpiresAt.toISOString());
    return access_token;
  } catch (error) {
    console.error('[Yahoo Token] Refresh exception:', error);
    return accessToken; // Return expired token as last resort
  }
}
