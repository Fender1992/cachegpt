...
export default function JiraIntegrationCard({ userId }: JiraIntegrationCardProps) {
  ...
  useEffect(() => {
    ...
    const error = params.get('jira_error');
    if (error) {
      setMessage({ type: 'error', text: `Jira connection failed: ${error}` });
      window.history.replaceState({}, '', '/settings');
    }
  }, [fetchStatus]);
  ...
  const handleConnect = () => {
    ...
    window.location.href = `https://auth.atlassian.com/authorize?audience=api.atlassian.com&client_id=${clientId}&scope=${encodeURIComponent(scope)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&prompt=consent`;
  };
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-500">Loading Jira integration...</span>
        </div>
      </div>
    );
  }
      ...
      {status?.connected ? (
        <>
          <div className="space-y-3 mb-4">
            ...
            {status.siteName && (
              <div className="flex items-center gap-2 text-sm">
                <span className="text-gray-600 dark:text-gray-400 ml-6">Site:</span>
                <span className="font-medium text-gray-900 dark:text-white">
                  {status.siteName}
                </span>
              </div>
        ...
        <>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Connect Jira to browse projects, search issues, and get AI context about your tickets.
          </p>
          <button
      ...
      <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
            <p>When you mention tickets or ask about your work, we search Jira on demand.</p>
            <p>Requires a Jira Cloud OAuth app with <code>read:jira-work</code> and <code>read:jira-user</code> scopes.</p>
          </div>
        </div>
      </div>
    </div>
