...
export default function GmailIntegrationCard({ userId }: GmailIntegrationCardProps) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      const res = await apiFetch('/api/integrations/gmail', { headers });
      if (res.ok) {
        const data: GmailIntegrationStatus = await res.json();
        setStatus(data);
      }
    } catch {
      // Silently fail
  ...
  useEffect(() => {
    ...
    if (error) {
      setMessage({ type: 'error', text: `Gmail connection failed: ${error}` });
      window.history.replaceState({}, '', '/settings');
    }
  }, [fetchStatus]);
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const headers = await getAuthHeaders();
      const res = await apiFetch('/api/integrations/gmail', {
        method: 'DELETE',
        headers
      });
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-500">Loading Gmail integration...</span>
        </div>
      </div>
    );
  }
      ...
      {status?.connected ? (
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="grid grid-cols-2 gap-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
              ...
              <div className="text-center">
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">
                  {status.providerData?.messagesTotal || 0}
                </p>
              </div>
            </div>
          </div>
        ...
        <>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Connect your Gmail account to get AI responses that understand your email conversations,
            threads, and context.
          </p>
          <button
