/**
 * Yahoo Mail Integration Status API
 * GET: Retrieve Yahoo integration status
 * DELETE: Disconnect Yahoo integration
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      labelCount: integration.provider_data?.label_count || 0,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch Yahoo status' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!integration) {
      return NextResponse.json({ message: 'No Yahoo integration found' });
    }
    ...
    return NextResponse.json({ message: 'Yahoo integration disconnected' });
  } catch (error) {
